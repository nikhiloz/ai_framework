...
void save_model_weights(TransformerModel *model, const char *filename) {
    ...
    for (int i = 0; i < model->num_blocks; i++) {
        ...
        // LN1
        fwrite(tb->ln1.weight.data, sizeof(float), tb->ln1.weight.rows * tb->ln1.weight.cols, fp);
        ...
        // LN2
        fwrite(tb->ln2.weight.data, sizeof(float), tb->ln2.weight.rows * tb->ln2.weight.cols, fp);
    }
...
void load_model_weights(TransformerModel *model, const char *filename) {
    ...
    for (int i = 0; i < model->num_blocks; i++) {
        ...
        // LN1
        fread(tb->ln1.weight.data, sizeof(float), tb->ln1.weight.rows * tb->ln1.weight.cols, fp);
        ...
        // LN2
        fread(tb->ln2.weight.data, sizeof(float), tb->ln2.weight.rows * tb->ln2.weight.cols, fp);
    }
...
void load_llama_weights(TransformerModel *model, const char *filename) {
    FILE *fp = fopen(filename, "rb");
    if (!fp) {
        perror("Failed to open file for loading Llama weights");
        return;
    }
    ...
    for (int i = 0; i < model->num_blocks; i++) {
        ...
        // FFN (SwiGLU)
        fread(tb->W1.data, sizeof(float), tb->W1.rows * tb->W1.cols, fp);
        fread(tb->W3.data, sizeof(float), tb->W3.rows * tb->W3.cols, fp); // Added W3
        fread(tb->b1.data, sizeof(float), tb->b1.rows * tb->b1.cols, fp);
        fread(tb->W2.data, sizeof(float), tb->W2.rows * tb->W2.cols, fp);
        fread(tb->b2.data, sizeof(float), tb->b2.rows * tb->b2.cols, fp);

        // LN2 (RMSNorm - only weight/gamma)
        fread(tb->ln2.weight.data, sizeof(float), tb->ln2.weight.rows * tb->ln2.weight.cols, fp);
    }
